...
<%
    ...
    int shortExtra = Integer.parseInt(request.getParameter("shortExtra").trim());
...
<%
	...
	List<Stock> list1 = new StockDS().getStockDifferanceBetweenDates(fromDate,toDate,shortExtra);
	if(!(list1.isEmpty()))
	{		
%>
  		<center>
  		<h3><%if(shortExtra == 1){ out.print("Short");}else{ out.print("Extra");} %> Stock रिपोर्ट : <font face="Kiran"><%=fd%></font><span class="fontsize12">/</span><font face="Kiran"><%=fm%></font><span class="fontsize12">/</span><font face="Kiran"><%=fy%></font>
...
<%   
        for(Stock s : list1 )
        {
            allSum.setMungdaal(allSum.getMungdaal()+ s.getMungdaal());
            allSum.setMatki(allSum.getMatki()+ s.getMatki());
            allSum.setMung(allSum.getMung()+ s.getMung());
            allSum.setMasuldaal(allSum.getMasuldaal()+ s.getMasuldaal());
            allSum.setHarbara(allSum.getHarbara()+ s.getHarbara());
            allSum.setVatana(allSum.getVatana()+ s.getVatana());
            allSum.setExtra1(allSum.getExtra1()+ s.getExtra1());
            allSum.setExtra2(allSum.getExtra2()+ s.getExtra2());
            allSum.setExtra3(allSum.getExtra3()+ s.getExtra3());
            allSum.setExtra4(allSum.getExtra4()+ s.getExtra4());
            allSum.setExtra5(allSum.getExtra5()+ s.getExtra5());
            allSum.setExtra6(allSum.getExtra6()+ s.getExtra6());
            allSum.setChvli(allSum.getChvli()+ s.getChvli());
            allSum.setTel(allSum.getTel()+ s.getTel());
            allSum.setMith(allSum.getMith()+ s.getMith());
            allSum.setMirchi(allSum.getMirchi()+ s.getMirchi());
            allSum.setHalad(allSum.getHalad()+ s.getHalad());
            allSum.setJire(allSum.getJire()+ s.getJire());
            allSum.setMohari(allSum.getMohari()+ s.getMohari());
            allSum.setTandul(allSum.getTandul()+ s.getTandul());
            
            StringTokenizer st3 = new StringTokenizer(s.getStockDiffDate(), "/- ");
   		    td = st3.nextToken();
   		    tm = st3.nextToken();
   		    ty = st3.nextToken();    
%>
