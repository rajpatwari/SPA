Ext.onReady(function()
{
	...
	prodSummary = new Ext.data.JsonStore(
	{
        ...
        root				: 	'root',
        idProperty 			: 	'dispatchTalukaOrderID',
        fields				: 	
        [
	...
	*/
    ...
    allTalukaOrderStore= new Ext.data.JsonStore(
    {
        ...
        root				: 	'root',
        idProperty 			: 	'rationID',
        fields				: 	['detailID','rationID','rationNameFull','talukaOrderID','dispatchTalukaOrderID','talukaOrderQty','previousDispatchedQty','currentStock','pendingOrderQty','todaysDispatch'],
        sortInfo			: 
        {
            field				:	'rationID',
            direction			:	'ASC'
    ...
    openOrderWin =  function ()
    {
        ...
        prodSummary.reload({params:{dispatchID:id},waitMsg:'Loading...'});
    };
	            ...
	            if(addSectionForm.getForm().isValid())
	            {
	                addSectionForm.getForm().submit(
	                {
	                    ...
	                    success				: 	function(form, action)
	                    {
	                        ...
	                        Ext.Msg.alert('Success', 'Taluka order added to dispatch');
	                        //win.hide();
	                    },
	                    failure: function(form, action)
	                    {
	                        Ext.Msg.alert('Error', 'Error occurred in adding taluka order to dispatch');
	                    }
	                });
	            }
	        }
	...
	{                   
        ...
        },'->',
        {
            ...
            handler 			: 	function()
            {
                ...
                for(var j=0;j<changedRecData.length;j++)
                {
                    ...
                    //var talukaOrderQty = item.data.talukaOrderQty;
                    var pendingOrderQty = item.data.pendingOrderQty;
                    var currentStock = item.data.currentStock;
                    var todaysDispatch = item.data.todaysDispatch;
                    var rationNameFull = item.data.rationNameFull;
                    //var previousDispatchedQty = item.data.previousDispatchedQty;

                    if(todaysDispatch === 0)++zeroCount;
                    if(pendingOrderQty === 0 && todaysDispatch> 0 )
                    {
                        Ext.Msg.alert('Ration already dispatched', "Please change Todays Dispatch for ration "+rationNameFull  );
                        return;
                    }
                    if(todaysDispatch > pendingOrderQty)
                    {
                        Ext.Msg.alert('Change Todays Dispatch', "Please change Todays Dispatch for ration "+rationNameFull + "<br> Today's dispatch should be less that or equal to Pending Order " );
                        return;
                    }
                    if(todaysDispatch > currentStock)
                    {
                        Ext.Msg.alert('Change Todays Dispatch', "Please change Todays Dispatch for ration "+rationNameFull + "<br> Today's dispatch should be less that or equal to Current Stock " );
                        return;
                    }
        ...
        [
         	...
         	{	header: 'Ration ID', width: 80, dataIndex: 'rationID',align: 'center' },
            {	header: 'Order Item',   width: 80, dataIndex: 'rationNameFull',align: 'center'},
            {	header: 'Order Qty(Pouches)',width: 80,dataIndex: 'talukaOrderQty',align: 'center'},
            {	header: 'Prev. Dispatched Qty(Pouches)',width: 80,dataIndex: 'previousDispatchedQty',  align: 'center' },
            {	header: 'Pending Order(Pouches)', width: 80,dataIndex: 'pendingOrderQty',align: 'center'},
            {	header: 'Current Stock In Godown(Pouches)', width: 80, dataIndex: 'currentStock', align: 'center' },                        
            { 	header:  "Today's Dispatch(Pouches)",width: 80, dataIndex: 'todaysDispatch',align: 'center',editor: new Ext.form.NumberField({ allowBlank: false,allowNegative: false,selectOnFocus : true }) }
        ]
    });
	...
	fromProductionBatchesGrid = new Ext.grid.GridPanel(
	{
        ...
        },'->',
        {
            ...
            handler 			: 	function()
            {
                ...
                var dispatchTalukaOrderID = sel[0].get('dispatchTalukaOrderID');
                    ...
                    fn					: 	function(btn) 
                    {
                        if (btn == 'yes') 
                        {
                            Ext.Ajax.request(
                            {
                                ...
                                params				: 	
                                {
                                    dispatchTalukaOrderID : 	dispatchTalukaOrderID
                                },
                                success				: 	function(resp,opt) 
                                {
        ...
        listeners 			: 	
        {
            rowclick 			: 	function (g, rowIndex,e) 
            {
                var sm = fromProductionBatchesGrid.getSelectionModel();
                var sel = sm.getSelections();
                var id = sel[0].get('dispatchTalukaOrderID');
                //var talukaMarathi = sel[0].get('talukaMarathi');
                //var orderNumber = sel[0].get('orderNumber');
                var talukaOrderID = sel[0].get('talukaOrderID');
                var biltyNo = sel[0].get('biltyNo');
                var biltyDate = sel[0].get('biltyDate');
                var districtID = sel[0].get('districtID');
        ...
        [
         	new Ext.grid.RowNumberer(),
         	{	header: 'dispatchTalukaOrderID',width: 100, sortable: true, dataIndex: 'dispatchTalukaOrderID',align: 'center' },
            ...
            {	header: 'Total Taluka Loading(tons)',width: 140, sortable: true,dataIndex: 'talukaTotalLoad', summaryType: 'sum', align: 'center', renderer : fourDecimalNum },
            {	header: 'Prev. Taluka Loading Dispatched(tons)', width: 140,sortable: true, dataIndex: 'previousDispatch',align: 'center', summaryType: 'sum', align: 'center', renderer : fourDecimalNum},
            {	header: 'Taluka Loading Pending(tons)',width: 140, sortable: true,dataIndex: 'talukaLoadPending',align: 'center',summaryType: 'sum', align: 'center',renderer : fourDecimalNum},
                            ...
                            if (btn == 'yes') 
                            {
                                ...
                                Ext.Ajax.request(
                                {
                                    ...
                                    success			: 	function(form, action)
                                    {
                                        ...
                                        Ext.Msg.alert(Ext.decode(form.responseText).header, Ext.decode(form.responseText).msg );
                                    },
                                    failure			: 	function(form, action)
                                    {
                                        Ext.Msg.alert(Ext.decode(form.responseText).header, Ext.decode(form.responseText).msg );
